...
class SetGenerator():
    def __init__(self, train_batch_size, test_batch_size) -> None:
        ...
        self.basic_transforms = A.Compose(
            [
                A.OneOf([
                    ...
                    A.RandomBrightnessContrast(p=0.5), ], p=0.3),
        ...
        self.training_set = Cifar10SearchDataset(root='./data', train=True,
                                                         download=True, transform=self.basic_transforms)

        self.test_set = Cifar10SearchDataset(root='./data', train=False,
                                                     download=True, transform=self.basic_transforms)
...
class JNetTrainer:
    def __init__(self, SetGenerator, n_epochs, dropout, save_path, save=False) -> None:
        ...
        self.optimizer = optim.SGD(
            self.net.parameters(), lr=0.0001, momentum=0.9, weight_decay=1e-4)
    ...
    def train(self):
        ...
        for epoch in epochs:  # loop over the dataset multiple times
            ...
            with torch.no_grad():
                ...
                self.train_accuracy_list[epoch] = 100 * \
    ...
    def plot_metrics(self):
        ...
        ax[1].set(title="Accuracy", xlabel="n_epochs",
    ...
    def get_feature_reduction(self):
        ...
        self.net.eval()
        for data in self.SetGenerator.train_split:
            # print(data)
    ...
    def get_test_predictions(self):
        ...
        with torch.no_grad():
            self.net.eval()
            for data in self.SetGenerator.test_data_loader:
                images, labels = data[0].to(
                    self.device), data[1].to(self.device)
    ...
    def get_metrics(self):
        y_pred, y = self.get_test_predictions()
        classes = self.SetGenerator.classes
        global_acc = metrics.accuracy_score(y_pred, y)
        for k in range(len(classes)):
            idx = np.argwhere(y == k)
            class_y = y[idx] == k
            class_y_pred = y_pred[idx] == k
            class_acc = metrics.accuracy_score(class_y, class_y_pred)
            class_precision = metrics.precision_score(class_y, class_y_pred)
            class_recall = metrics.recall_score(class_y, class_y_pred)
            class_F1 = metrics.f1_score(class_y, class_y_pred)
            row = [classes[k], class_acc,
        ...
        sns.heatmap(confusion_matrix/np.sum(confusion_matrix), annot=True,
                    cmap='magma', xticklabels=classes, yticklabels=classes, norm=LogNorm())
        return fig
